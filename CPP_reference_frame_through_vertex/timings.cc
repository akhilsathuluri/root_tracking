...
int main(int argc, char const *argv[])
{
  ...
  VectorXcd phii2(18), tempNRC(18), tempDMC(18), tempNNC(18);
  phii << 0.2,0,1.28,0.2,0,0,
   ...
   phii2 << std::complex<double>(1.8700438348728161,0),std::complex<double>(-0.5235870043873335,0),std::complex<double>(0.,-0.926898973258719),std::complex<double>(0.,-0.10569096861540776),
      std::complex<double>(0.,0.7154784037290103),std::complex<double>(0,0),std::complex<double>(1.5707963267948966,1.5262959577414468),
  ...
  std::cout << "Computing the average time taken by the corresponding trackers over 1000 runs for real roots only" << std::endl;
...
std::cout << "Real only NRTracker: "<< times.mean() << std::endl;
...
std::cout << "Real only DMTracker: "<< times.mean() << std::endl;
...
for (int t = 0; t < 1000; t++) {
  ...
  std::cout << "Real only NNTracker: "<< times.mean() << std::endl;
...
int singular_iter = legvals.rows();

  std::cout << "Computing the average time taken by the corresponding trackers over 1000 runs by treating all roots as complex" << std::endl;

  MatrixXcd solsNRC(18,1);
  solsNRC << phii2;
  tempNRC = phii2;
  ...
  std::cout << "Complex roots NRTracker: "<< times.mean() << std::endl;
  ...
  for (int t = 0; t < 1000; t++) {
    ...
    for (int i = 1; i < singular_iter; i++){
      ...
      Choosing small enough `eps` makes DMTracker work like an NRTracker
      */
      tempDMC = rt.DMCTracker(Clegvals.row(i-1), Clegvals.row(i), tempDMC, JetaextthetaC, JetaextphiC, 0.05, etaextC);
  ...
  std::cout << "Complex roots DMTracker: "<< times.mean() << std::endl;
    ...
    MatrixXcd solsNNC(18,1), tempFKC(14, 18);
    solsNNC << phii2;
    tempNNC = phii2;
    // Initialise all the known roots
    tempFKC = initsols2;
  ...
  for (int t = 0; t < 1000; t++) {
    ...
    std::cout << "Complex roots NNTracker: "<< times.mean() << std::endl;
