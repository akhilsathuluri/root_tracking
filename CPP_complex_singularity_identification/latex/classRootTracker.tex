\hypertarget{classRootTracker}{}\doxysection{Root\+Tracker Class Reference}
\label{classRootTracker}\index{RootTracker@{RootTracker}}


{\ttfamily \#include $<$root\+\_\+tracker.\+hh$>$}

\doxysubsection*{Public Member Functions}
\begin{DoxyCompactItemize}
\item 
int \mbox{\hyperlink{classRootTracker_a0d29be3a24392b6a864b8e471f23db6c}{Methods}} ()
\item 
Vector\+Xd \mbox{\hyperlink{classRootTracker_a6bb3dcb73ea58ac44ab15d7803de3d5a}{NRTracker}} (Vector\+Xd x, Vector\+Xd y, std\+::function$<$ Vector\+Xd(Vector\+Xd)$>$ f, std\+::function$<$ Matrix\+Xd(Vector\+Xd)$>$ Jfy, double eps=pow(10, -\/10))
\item 
Vector\+Xd \mbox{\hyperlink{classRootTracker_acc5d4b8e89ace70cee9c227f033e5a92}{DMTracker}} (Vector\+Xd xprev, Vector\+Xd x, Vector\+Xd y, std\+::function$<$ Matrix\+Xd(Vector\+Xd)$>$ Jfx, std\+::function$<$ Matrix\+Xd(Vector\+Xd)$>$ Jfy, double eps=0, std\+::function$<$ Vector\+Xd(Vector\+Xd)$>$ f=NULL)
\item 
Vector\+Xd \mbox{\hyperlink{classRootTracker_ae13e787a2ef083592f5c8d07ac4d08bc}{NNTracker}} (Vector\+Xd ys, Matrix\+Xd ysols, int index)
\item 
Matrix\+Xd \mbox{\hyperlink{classRootTracker_a8762c10ea3a49ae3410fcec863cf08d4}{track\+All\+Branches}} (Vector\+Xd x, Matrix\+Xd y, std\+::function$<$ Vector\+Xd(Vector\+Xd)$>$ f, std\+::function$<$ Matrix\+Xd(Vector\+Xd)$>$ Jfy)
\item 
Vector\+Xcd \mbox{\hyperlink{classRootTracker_a81f1227c2436cf1a84a7b8608b683bdd}{NRCTracker}} (Vector\+Xcd x, Vector\+Xcd y, std\+::function$<$ Vector\+Xcd(Vector\+Xcd)$>$ f, std\+::function$<$ Matrix\+Xcd(Vector\+Xcd)$>$ Jfy, double eps=pow(10, -\/10))
\item 
Vector\+Xcd \mbox{\hyperlink{classRootTracker_aa6467ae384a2333b03f7432da49513a0}{DMCTracker}} (Vector\+Xcd xprev, Vector\+Xcd x, Vector\+Xcd y, std\+::function$<$ Matrix\+Xcd(Vector\+Xcd)$>$ Jfx, std\+::function$<$ Matrix\+Xcd(Vector\+Xcd)$>$ Jfy, double eps=0, std\+::function$<$ Vector\+Xcd(Vector\+Xcd)$>$ f=NULL)
\item 
Vector\+Xcd \mbox{\hyperlink{classRootTracker_a73c068967c8f4403f9a0ec1c703c2be8}{NNCTracker}} (Vector\+Xcd ys, Matrix\+Xcd ysols, int index)
\item 
int \mbox{\hyperlink{classRootTracker_a9b24fe40d4f662b984311365ffd07aa2}{SEI}} (Matrix\+Xd allroots, double alpha, int selectedroot, std\+::function$<$ Vector\+Xd(double)$>$ \mbox{\hyperlink{manipulator_8hh_a617dcde540c04aa64274710efb425bb7}{compute\+Xfrom\+Param}}, Ref$<$ Vector\+Xd $>$ alphahist, Ref$<$ Matrix\+Xd $>$ disthist, std\+::function$<$ Vector\+Xd(Vector\+Xd)$>$ f, std\+::function$<$ Matrix\+Xd(Vector\+Xd)$>$ Jfy, std\+::function$<$ Vector\+Xd(double)$>$ \mbox{\hyperlink{manipulator_8hh_a1c6194cffda1938f04d3178b7f4b3f1a}{computeq\+Extfrom\+Param}})
\end{DoxyCompactItemize}


\doxysubsection{Detailed Description}
The \mbox{\hyperlink{classRootTracker}{Root\+Tracker}} class consists of implementations of the following\+:
\begin{DoxyItemize}
\item Newton-\/\+Raphson method based tracking or NRTracker
\item Davidenkos method based tracking or DMTracker
\item Nearest neighbour based tracking or NNTracker

\begin{DoxyRefDesc}{Todo}
\item[\mbox{\hyperlink{todo__todo000007}{Todo}}]See how things change when open\+MP is enabled with Eigen 

See how things change when BLAS and LAPACK are used with Eigen 

Implement an event identification method that can handle or atleast alert when the system moves close to a singularity\end{DoxyRefDesc}


\begin{DoxyRefDesc}{Todo}
\item[\mbox{\hyperlink{todo__todo000008}{Todo}}]modify the paper with a definition of distance as all the individual elements of the vector being within the eps radius ball. So once we define the eps, we take 2 steps, which gives us 4 solution values, i.\+e. two branches two sols. Then use a multivariable interpolation scheme to interpolate from both sides. Then find their intersection to find the location of singularity. \end{DoxyRefDesc}

\end{DoxyItemize}

\doxysubsection{Member Function Documentation}
\mbox{\Hypertarget{classRootTracker_aa6467ae384a2333b03f7432da49513a0}\label{classRootTracker_aa6467ae384a2333b03f7432da49513a0}} 
\index{RootTracker@{RootTracker}!DMCTracker@{DMCTracker}}
\index{DMCTracker@{DMCTracker}!RootTracker@{RootTracker}}
\doxysubsubsection{\texorpdfstring{DMCTracker()}{DMCTracker()}}
{\footnotesize\ttfamily Vector\+Xcd Root\+Tracker\+::\+DMCTracker (\begin{DoxyParamCaption}\item[{Vector\+Xcd}]{xprev,  }\item[{Vector\+Xcd}]{x,  }\item[{Vector\+Xcd}]{y,  }\item[{std\+::function$<$ Matrix\+Xcd(Vector\+Xcd)$>$}]{Jfx,  }\item[{std\+::function$<$ Matrix\+Xcd(Vector\+Xcd)$>$}]{Jfy,  }\item[{double}]{eps = {\ttfamily 0},  }\item[{std\+::function$<$ Vector\+Xcd(Vector\+Xcd)$>$}]{f = {\ttfamily NULL} }\end{DoxyParamCaption})}

The DMCTracker uses the Davindenkos\textquotesingle{} integration method to find the solutions satisfied by the constrain equations. \mbox{\Hypertarget{classRootTracker_acc5d4b8e89ace70cee9c227f033e5a92}\label{classRootTracker_acc5d4b8e89ace70cee9c227f033e5a92}} 
\index{RootTracker@{RootTracker}!DMTracker@{DMTracker}}
\index{DMTracker@{DMTracker}!RootTracker@{RootTracker}}
\doxysubsubsection{\texorpdfstring{DMTracker()}{DMTracker()}}
{\footnotesize\ttfamily Vector\+Xd Root\+Tracker\+::\+DMTracker (\begin{DoxyParamCaption}\item[{Vector\+Xd}]{xprev,  }\item[{Vector\+Xd}]{x,  }\item[{Vector\+Xd}]{y,  }\item[{std\+::function$<$ Matrix\+Xd(Vector\+Xd)$>$}]{Jfx,  }\item[{std\+::function$<$ Matrix\+Xd(Vector\+Xd)$>$}]{Jfy,  }\item[{double}]{eps = {\ttfamily 0},  }\item[{std\+::function$<$ Vector\+Xd(Vector\+Xd)$>$}]{f = {\ttfamily NULL} }\end{DoxyParamCaption})}

The DMTracker uses the Davindenkos\textquotesingle{} integration method to find the solutions satisfied by the constrain equations. The output is the values of the unknown variables at each tracking step. The problem of tracking is solved as an initial value problem using the first order derivative form of the constraint equations. This function accepts functions as arguments using the {\ttfamily C++11} style functional library. This method by default uses the Explicit Euler integration scheme and is the only supported scheme currently. 
\begin{DoxyParams}{Parameters}
{\em xprev} & The set of input/known variables at the previous tracking step \\
\hline
{\em x} & The set of input/known variables at the current tracking step \\
\hline
{\em y} & The set of output/unknown variables at current step \\
\hline
{\em Jfx} & The expression of the Jacobian matrix of f with respect to x, the known variables. Takes in a single argument of type {\ttfamily Vector\+Xd} consisting y, x and outputs the evaluation of Jfx \\
\hline
{\em Jfy} & The expression of the Jacobian matrix of f with respect to y, the unknown variables. Takes in a single argument of type {\ttfamily Vector\+Xd} consisting y, x and outputs the evaluation of Jfy. \\
\hline
{\em eps} & The tolerance of {\ttfamily drift} to which the computed solutions are to satisfy the non-\/linear equations. If the drift exceeds the given tolerance, a Newton-\/\+Raphson (NR) step is used to bring the variables back to the constraint manifold. The default value of {\ttfamily eps} is set to 0, meaning the NR step correction is {\ttfamily off} by default. \\
\hline
{\em f} & The set of expressions of the non-\/linear functions relating x, y. This parameter is only required if {\ttfamily eps} is defined. \\
\hline
\end{DoxyParams}
\begin{DoxyRefDesc}{Todo}
\item[\mbox{\hyperlink{todo__todo000004}{Todo}}]Provide support for different integration methods 

This isnt working. Implement it using prevtheta like in Mathematica rather than using xnext. That should fix it. 

The same scheme estimating dx as xnext-\/x is unstable \end{DoxyRefDesc}
\mbox{\Hypertarget{classRootTracker_a0d29be3a24392b6a864b8e471f23db6c}\label{classRootTracker_a0d29be3a24392b6a864b8e471f23db6c}} 
\index{RootTracker@{RootTracker}!Methods@{Methods}}
\index{Methods@{Methods}!RootTracker@{RootTracker}}
\doxysubsubsection{\texorpdfstring{Methods()}{Methods()}}
{\footnotesize\ttfamily int Root\+Tracker\+::\+Methods (\begin{DoxyParamCaption}{ }\end{DoxyParamCaption})}

The methods function can be called to view names of all the implemented root tracking methods. \begin{DoxyRefDesc}{Todo}
\item[\mbox{\hyperlink{todo__todo000001}{Todo}}]Give provision for an FK solver along with root trackers to use when trackers fail. \end{DoxyRefDesc}
\mbox{\Hypertarget{classRootTracker_a73c068967c8f4403f9a0ec1c703c2be8}\label{classRootTracker_a73c068967c8f4403f9a0ec1c703c2be8}} 
\index{RootTracker@{RootTracker}!NNCTracker@{NNCTracker}}
\index{NNCTracker@{NNCTracker}!RootTracker@{RootTracker}}
\doxysubsubsection{\texorpdfstring{NNCTracker()}{NNCTracker()}}
{\footnotesize\ttfamily Vector\+Xcd Root\+Tracker\+::\+NNCTracker (\begin{DoxyParamCaption}\item[{Vector\+Xcd}]{ys,  }\item[{Matrix\+Xcd}]{ysols,  }\item[{int}]{index }\end{DoxyParamCaption})}

The NNCTracker uses the nearest neighbour method to identify the roots belonging to a required branch. \mbox{\Hypertarget{classRootTracker_ae13e787a2ef083592f5c8d07ac4d08bc}\label{classRootTracker_ae13e787a2ef083592f5c8d07ac4d08bc}} 
\index{RootTracker@{RootTracker}!NNTracker@{NNTracker}}
\index{NNTracker@{NNTracker}!RootTracker@{RootTracker}}
\doxysubsubsection{\texorpdfstring{NNTracker()}{NNTracker()}}
{\footnotesize\ttfamily Vector\+Xd Root\+Tracker\+::\+NNTracker (\begin{DoxyParamCaption}\item[{Vector\+Xd}]{ys,  }\item[{Matrix\+Xd}]{ysols,  }\item[{int}]{index }\end{DoxyParamCaption})}

The NNTracker uses the nearest neighbour method to identify the roots belonging to a required branch. The output is the selected root at each tracking step. This method assumes the existance of a solver method, {\ttfamily Solve}, which computes all the roots for given input variables, {\ttfamily x}. The function expects the solutions to be ordered with all the reals together and the variables belonging to S1 together.


\begin{DoxyParams}{Parameters}
{\em ys} & The initiation of the known root of the required branch \\
\hline
{\em ysols} & All the solutions obtained by the {\ttfamily Solve} method used \\
\hline
{\em index} & The index upto which the reals are present.\\
\hline
\end{DoxyParams}
\begin{DoxyRefDesc}{Todo}
\item[\mbox{\hyperlink{todo__todo000005}{Todo}}]The method currently deals only with variables belonging to R and S1. 

Treating the Rodriques parameters as locally belonging to R. 

Add assertions for cols of ys and ysols to be same. \end{DoxyRefDesc}
\mbox{\Hypertarget{classRootTracker_a81f1227c2436cf1a84a7b8608b683bdd}\label{classRootTracker_a81f1227c2436cf1a84a7b8608b683bdd}} 
\index{RootTracker@{RootTracker}!NRCTracker@{NRCTracker}}
\index{NRCTracker@{NRCTracker}!RootTracker@{RootTracker}}
\doxysubsubsection{\texorpdfstring{NRCTracker()}{NRCTracker()}}
{\footnotesize\ttfamily Vector\+Xcd Root\+Tracker\+::\+NRCTracker (\begin{DoxyParamCaption}\item[{Vector\+Xcd}]{x,  }\item[{Vector\+Xcd}]{y,  }\item[{std\+::function$<$ Vector\+Xcd(Vector\+Xcd)$>$}]{f,  }\item[{std\+::function$<$ Matrix\+Xcd(Vector\+Xcd)$>$}]{Jfy,  }\item[{double}]{eps = {\ttfamily pow(10,~-\/10)} }\end{DoxyParamCaption})}

The NRCTracker uses the Newton-\/\+Raphson method iteratively to find the solutions satisfied by the constrain equations. \begin{DoxyRefDesc}{Todo}
\item[\mbox{\hyperlink{todo__todo000003}{Todo}}]Write overloaded function for save\+Date 

Write overloaded function for eta and J 

Make the tracker itself overloaded \end{DoxyRefDesc}
\mbox{\Hypertarget{classRootTracker_a6bb3dcb73ea58ac44ab15d7803de3d5a}\label{classRootTracker_a6bb3dcb73ea58ac44ab15d7803de3d5a}} 
\index{RootTracker@{RootTracker}!NRTracker@{NRTracker}}
\index{NRTracker@{NRTracker}!RootTracker@{RootTracker}}
\doxysubsubsection{\texorpdfstring{NRTracker()}{NRTracker()}}
{\footnotesize\ttfamily Vector\+Xd Root\+Tracker\+::\+NRTracker (\begin{DoxyParamCaption}\item[{Vector\+Xd}]{x,  }\item[{Vector\+Xd}]{y,  }\item[{std\+::function$<$ Vector\+Xd(Vector\+Xd)$>$}]{f,  }\item[{std\+::function$<$ Matrix\+Xd(Vector\+Xd)$>$}]{Jfy,  }\item[{double}]{eps = {\ttfamily pow(10,~-\/10)} }\end{DoxyParamCaption})}

The NRTracker uses the Newton-\/\+Raphson method iteratively to find the solutions satisfied by the constrain equations. The output is the values of the unknown variables at each tracking step. This function accepts functions as arguments using the {\ttfamily C++11} style functional library. 
\begin{DoxyParams}{Parameters}
{\em x} & The set of input/known variables at current tracking step \\
\hline
{\em y} & The set of output/unknown variables at current step \\
\hline
{\em f} & The set of expressions of the non-\/linear functions relating x, y. Takes in a single argument of type {\ttfamily Vector\+Xd} consisting y, x and outputs the evaluation of f. \\
\hline
{\em Jfy} & The expression of the Jacobian matrix of f with respect to y, the unknown variables. Takes in a single argument of type {\ttfamily Vector\+Xd} consisting y, x and outputs the evaluation of Jfy. \\
\hline
{\em eps} & The required tolerance to which the computed solutions are to satisfy the non-\/linear equations. Default value is set to 10$^\wedge$-\/10 \\
\hline
\end{DoxyParams}
\begin{DoxyRefDesc}{Todo}
\item[\mbox{\hyperlink{todo__todo000002}{Todo}}]Decide to return q or y in the NRTracker method 

fval returning nan is not being handled in the while loop 

Input argument format and other relevant checks \end{DoxyRefDesc}
\mbox{\Hypertarget{classRootTracker_a9b24fe40d4f662b984311365ffd07aa2}\label{classRootTracker_a9b24fe40d4f662b984311365ffd07aa2}} 
\index{RootTracker@{RootTracker}!SEI@{SEI}}
\index{SEI@{SEI}!RootTracker@{RootTracker}}
\doxysubsubsection{\texorpdfstring{SEI()}{SEI()}}
{\footnotesize\ttfamily int Root\+Tracker\+::\+SEI (\begin{DoxyParamCaption}\item[{Matrix\+Xd}]{allroots,  }\item[{double}]{alpha,  }\item[{int}]{selectedroot,  }\item[{std\+::function$<$ Vector\+Xd(double)$>$}]{compute\+Xfrom\+Param,  }\item[{Ref$<$ Vector\+Xd $>$}]{alphahist,  }\item[{Ref$<$ Matrix\+Xd $>$}]{disthist,  }\item[{std\+::function$<$ Vector\+Xd(Vector\+Xd)$>$}]{f,  }\item[{std\+::function$<$ Matrix\+Xd(Vector\+Xd)$>$}]{Jfy,  }\item[{std\+::function$<$ Vector\+Xd(double)$>$}]{computeq\+Extfrom\+Param }\end{DoxyParamCaption})}

The SEI uses the distance between different branches of the solutions to identify and estimate the singular configuration of the given set of non-\/linear equations. Further, it uses a quadratic extrapolation scheme to estimate the singular configuration. This function needs the computation of all the roots (currently handles only real roots).


\begin{DoxyParams}{Parameters}
{\em allroots} & Solutions of all the branches of the non-\/linear equations \\
\hline
{\em alpha} & Parameter value \\
\hline
{\em selectedroot} & The index of the selected branch (Use index starting with 1) \\
\hline
{\em compute\+Xfrom\+Param} & Compute the values of the input variables based on the path parametrisation \\
\hline
{\em alphahist} & Input vector (global to main) to store history of alphas for estimating the singular configuration \\
\hline
{\em disthist} & Input vector (global to main) to store history of distances for estimating the singular configuration \\
\hline
{\em f} & Set of non-\/linear equations \\
\hline
{\em Jft} & Jacobian matrix of the set of equations \\
\hline
{\em computeq\+Extfrom\+Param} & Function which produces the extended configuration values given the path parameter\\
\hline
\end{DoxyParams}
\begin{DoxyRefDesc}{Todo}
\item[\mbox{\hyperlink{todo__todo000006}{Todo}}]Optionally Bertini can be used to compute all the roots. Note that the method is limited to single parameter paths. 

Check the alpha selection with Aditya 

Assumes a quadratic interpolation, but can be made to accept an nth degree polynomial \end{DoxyRefDesc}
\mbox{\Hypertarget{classRootTracker_a8762c10ea3a49ae3410fcec863cf08d4}\label{classRootTracker_a8762c10ea3a49ae3410fcec863cf08d4}} 
\index{RootTracker@{RootTracker}!trackAllBranches@{trackAllBranches}}
\index{trackAllBranches@{trackAllBranches}!RootTracker@{RootTracker}}
\doxysubsubsection{\texorpdfstring{trackAllBranches()}{trackAllBranches()}}
{\footnotesize\ttfamily Matrix\+Xd Root\+Tracker\+::track\+All\+Branches (\begin{DoxyParamCaption}\item[{Vector\+Xd}]{x,  }\item[{Matrix\+Xd}]{y,  }\item[{std\+::function$<$ Vector\+Xd(Vector\+Xd)$>$}]{f,  }\item[{std\+::function$<$ Matrix\+Xd(Vector\+Xd)$>$}]{Jfy }\end{DoxyParamCaption})}

The track\+All\+Branches routine tracks one step in all the branches given the next steps input variable and an initial guess of the unknown variables using the relating function f and its Jacobian Jfy. 

The documentation for this class was generated from the following files\+:\begin{DoxyCompactItemize}
\item 
\mbox{\hyperlink{root__tracker_8hh}{root\+\_\+tracker.\+hh}}\item 
\mbox{\hyperlink{root__tracker_8cc}{root\+\_\+tracker.\+cc}}\end{DoxyCompactItemize}
