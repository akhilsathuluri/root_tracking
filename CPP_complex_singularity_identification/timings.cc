...
int singular_iter = 18;
  ...
  for (int t = 0; t < 1000; t++) {
    ...
    for (int i = 1; i < singular_iter; i++){
      tempNRC = rt.NRCTracker(legvals.row(i), tempNRC, etaextC, JetaextphiC);
  ...
  for (int t = 0; t < 1000; t++) {
    ...
    for (int i = 1; i < singular_iter; i++){
      ...
      Choosing small enough `eps` makes DMTracker work like an NRTracker
      */
      tempDMC = rt.DMCTracker(legvals.row(i-1), legvals.row(i), tempDMC, JetaextthetaC, JetaextphiC, 0.05, etaextC);
  ...
  for (int t = 0; t < 1000; t++) {
    ...
    for (int i = 1; i < singular_iter; i++){
      for (int j = 0; j < tempFKC.rows(); j++) {
        tempFKC.row(j) = rt.NRCTracker(legvals.row(i), tempFKC.row(j), etaextC, JetaextphiC);
      }
